...
touchMapLite.prototype.getMarkersFormUrlParams = function () {
    var params, index, keyValue, markers, markersIndex, markerParams;
...
touchMapLite.prototype.marker = function (title, lat, lon, map, live, radius) {
    ...
    if (live) {
        ...
        for (id = 0; id < map.MARKERS.length; id++) {
...
touchMapLite.prototype.marker.prototype = {
    ...
    createGPSelement: function (accuracy) {
        var circle, ctx, image, radius, metersPerPixel;
        ...
        if (document.getElementById("accuracy")) {
            this.element.removeChild(document.getElementById("accuracy"));
        }
    ...
    hideBubbles: function () {
        var mm, bb;
        for (mm = document.getElementById('markers').firstChild; mm; mm = mm.nextSibling) {
            if (mm.className === 'marker') {
                for (bb = mm.firstChild; bb; bb = bb.nextSibling) {
